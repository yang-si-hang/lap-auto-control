...
void torqueControl(xMateErProRobot &robot)
{
  ...
  // xMateModel<7> modelmodel = robot.model();
  error_code ec;
  ...
  damping.setZero();
  damping.topLeftCorner(3, 3) << 3.0 * sqrt(translational_stiffness) *
                                     Eigen::MatrixXd::Identity(3, 3);
  damping.bottomRightCorner(3, 3) << 3.0 * sqrt(rotational_stiffness) *
                                         Eigen::MatrixXd::Identity(3, 3);
