...
int main()
{
  ...
  try
  {
    ...
    // rtCon->startReceiveRobotState({RtSupportedFields::jointPos_m, RtSupportedFields::tcpPose_m});
    std::array<double, 16> init_pos{}, end_pos{},my_pos{};
    ...
    std::function<CartesianPosition()> callback = [&, rtCon]()
    {
      ...
      rtCon->getStateData(RtSupportedFields::tcpPose_m, my_pos);
      cout<<my_pos<<endl;
