...
def cp_stop(pub,time=1):
    frequency = 1000
    msg_pub = PoseStamped()
    msg_pub.header.stamp = rospy.Time.now()
    msg_pub.pose.position.x = pose_now[0]
    msg_pub.pose.position.y = pose_now[1]
    msg_pub.pose.position.z = pose_now[2]
    ...
    for i in range(int(frequency*time)):
        pub.publish(msg_pub)
        rate.sleep()
