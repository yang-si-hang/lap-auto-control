'''
该文件用于求解眼在世界中固定的 手眼标定
根据测得的 机械臂末端变化矩阵、 相机坐标系中机械臂末端刚体变换矩阵
改写为AX=XB形式(将测得数据改写为A、B)
然后采用 shiu(A,B) 求解X的最小二乘解
...
def generate_transform_matrices(n):
    # 生成n个变换矩阵
    transform_matrices = [random_T() for _ in range(n)]
    return transform_matrices



T_0_rob = np.loadtxt(f'E:\\Download\\easy_handeye_demo-master\\easy_handeye_demo-master\\qualisys_rob_calibration\\T_0_rob.txt', delimiter=',')
T_cam_rigid = np.loadtxt(f'E:\\Download\\easy_handeye_demo-master\\easy_handeye_demo-master\\qualisys_rob_calibration\\T_cam_rigid.txt', delimiter=',')
n = T_0_rob.shape[1] // 4
A = np.zeros((4,4*(n-1)))
B = np.zeros((4,4*(n-1)))
for i in range(n-1):
    A[:,4*i:4*i+4] = T_0_rob[:,4*(i+1):4*(i+1)+4] @ np.linalg.inv(T_0_rob[:,4*i:4*i+4])
    B[:,4*i:4*i+4] = T_cam_rigid[:,4*(i+1):4*(i+1)+4] @ np.linalg.inv(T_cam_rigid[:,4*i:4*i+4])
