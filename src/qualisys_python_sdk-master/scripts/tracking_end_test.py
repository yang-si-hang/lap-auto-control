...
tracking_name = 'Z1'
tracking_file_path = '/home/irobotcare/桌面/EX_Data/lap/test/tracking_test.txt'

T_tracking = np.loadtxt(tracking_file_path, delimiter=',')
init_end_position = np.array([156, 164, 126, 1])
print(f'T_tracking\n{T_tracking}')
p_rigid_end = np.linalg.inv(T_tracking) @ init_end_position
...
async def main():
    ...
    def on_packet(packet):
        ...
        if rigid_name == tracking_name:
                # print(f'data of tracking: \n{data}\n')
                # print(f'R:\n{R}\n{[[position[0]],[position[1]],[position[2]]]}')
                T_tracking = np.concatenate((R.squeeze(), np.array([[position[0],position[1],position[2]]]).T), axis=1)
                T_tracking = np.concatenate((T_tracking, np.array([[0,0,0,1]])), axis=0)
                print(f'p_rigid_end\n{p_rigid_end}')
                print(f'T_tracking:\n{T_tracking}')
                print(f'p_end:\n{T_tracking @ p_rigid_end}')
