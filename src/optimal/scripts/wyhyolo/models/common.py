...
import sys
sys.path.append("/home/yiliao/wyh/laparoscope_ws/src/optimal/scripts/wyhyolo")
from utils.datasets import exif_size, letterbox
...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(self, weights='yolov5s.pt', device=None, dnn=True):
        ...
        if jit:  # TorchScript
            ...
            # model = torch.load(w) if 'torchscript' in w else attempt_load(weights, map_location=device)
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        for i, im in enumerate(imgs):
            ...
            if isinstance(im, (str, Path)):  # filename or uri
                ...
                im = np.asarray(exif_size(im))
            elif isinstance(im, Image.Image):  # PIL Image
                im, f = np.asarray(exif_size(im)), getattr(im, 'filename', f) or f
