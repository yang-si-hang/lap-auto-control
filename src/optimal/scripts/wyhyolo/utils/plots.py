...
class Annotator:
    ...
    def keypoint_circle(self, x, y, r):
        if self.pil:
            # 没用PIL修改了哦，这里是画框的时候把尖端也打个圈， 传入的一般不是pil图吧，先放这里
            self.draw.ellipse((x - r, y - r, x + r, y + r), width=2)
        else:  # cv
            cv2.circle(self.im, (x, y), 30, (250, 250, 0), 8)
...
def feature_visualization(x, module_type, stage, n=64, save_dir=Path('runs/detect/exp')):
    ...
    if 'Detect' not in module_type:
        ...
        if height > 1 and width > 1:
            f = save_dir / f"stage{stage}_{module_type.split('.')[-1]}_features{time.time():.6f}.png"  # filename
