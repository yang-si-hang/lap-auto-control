...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    # 获得烟雾模型
    fog_model = torch.load("my_diy_tool/binary_model/the0301_renet18_best.pt")
    fog_model.eval()

    with open("my_date/csvdata.csv", "w+", newline='') as csv_file:  # 打开csv格式文件
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            t2 = time_sync()
            dt[0] += t2 - t1
            start_one_img = time.time()
            # Inference
            visualize = increment_path(save_dir / Path(path).stem, mkdir=True) if visualize else False
            pred = model(im, augment=augment, visualize=visualize)
            detect_tool_time = time.time() - start_one_img
            print("detect_tool time :%.6f" % (detect_tool_time))
            # print("pred = ",pred)
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if set_fog == 1:
                    ...
                    temp_fog = cv2.resize(temp_fog, (400,200))
                    fog = numpy.mean(temp_fog)
                    fog = round(float(fog), 2)
                    set_fog = 0
                ...
                #     fog = round(fog, 2)
                #     set_fog = 0
                    # fog_time = time.time() - start_one_img - detect_tool_time
                    # print('fog_time:%.6f' % fog_time)
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        if save_img or save_crop or view_img:  # Add bbox to image
                            ...
                            # names = ['Right angle separating plier', 'Intestinal forceps', 'Ligation clips', 'electric hook', 'Separating plier', 'scissors']
                            #          350241
                            detect_tool.append(c)
                            num_tool += 1
                            tip_num = 0
                            # --------------------------  截取检测区域  ---------------------------------------------
                            # ------------------适度放大截取区域-----------------------------------------
                            if c == main_tool:
                                main_tool = -1
                                if xyxy[0].item() > 40:
                                    temp_x1 = int(xyxy[0].item() - 20)
                                else:
                                    temp_x1 = int(xyxy[0].item())

                                if xyxy[1].item() > 40:
                                    temp_y1 = int(xyxy[1].item() - 20)
                                else:
                                    temp_y1 = int(xyxy[1].item())

                                if xyxy[2].item() < 1880:
                                    temp_x2 = int(xyxy[2].item() + 20)
                                else:
                                    temp_x2 = int(xyxy[2].item())

                                if xyxy[3].item() < 1000:
                                    temp_y2 = int(xyxy[3].item() + 20)
                                else:
                                ...
                                # 交并比与极大值抑制
                                pred_tip = non_max_suppression(pred_tip, 0.20, iou_thres, classes, agnostic_nms,
                                                               max_det=max_det)
                                for i_tip, det_tip in enumerate(pred_tip):  # per image
                                    ...
                                    if len(det_tip):
                                        ...
                                        for *xyxy_tip, conf_tip, cls_tip in reversed(det_tip):
                                            ...
                                            tip_r = int(max((xyxy_tip[2].item() - xyxy_tip[0].item()), (
                    ...
                    # ------------选出目前的著观测器具--------------
                    if len(detect_tool) > 0:
                        main_tool_i = 0
                        over_tool = 0
                            # names = ['Right angle separating plier', 'Intestinal forceps', 'Ligation clips', 'electric hook', 'Separating plier', 'scissors']
                            #          350241  253041
                        while main_tool_i < 6:
                            for each in detect_tool:
                                if each == tool_id[main_tool_i]:
                                    main_tool = tool_id[main_tool_i]
                                    over_tool = 1
                            if over_tool == 1:
                                break
                            main_tool_i += 1
                ...
                # print("one img after detect_tip_fog use %.4fs" % (time.time() - start_one_img))
                one_time = time.time() - start_one_img
                one_time = round(one_time, 5)
                writer.writerow([str(one_time)])
                ...
                # if save_img:
                #     if fog < 0.6:      # 烟雾值大于0.6的时候文字变红
                #         fog_color = (255, 0, 255)
                #     else:
                #         fog_color = (0, 0, 255)
                #
                #     cv2.putText(im0, 'fog:' + str(fog), (10, 70), cv2.FONT_HERSHEY_PLAIN, 3,
                #                 fog_color, 3)
                #     fog = 0
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/202202/model7_6/dan_model7/weights/best.pt',
                        help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'my_date/2018-02-09_130837_VID002.mp4',
    ...
    parser.add_argument('--project', default=ROOT / 'runs/detect/my_paper', help='save results to project/name')
    parser.add_argument('--name', default='ooo', help='save results to project/name')
...
if __name__ == "__main__":
    ...
    opt = parse_opt()
    main(opt)
    print("All be processed in %.2f" % (time.time() - start))
