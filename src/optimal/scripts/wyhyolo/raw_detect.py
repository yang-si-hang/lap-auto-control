...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            imc = im0.copy() if save_crop else im0   # for save_crop 之前是if save_crop else
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'runs/train/sum82/weights/best.pt',
                        help='model path(s)')
    parser.add_argument('--source', type=str, default=ROOT / 'video', help='file/dir/URL/glob, 0 for webcam')
    ...
    parser.add_argument('--project', default=ROOT / 'runs/detect/detect0', help='save results to project/name')
    parser.add_argument('--name', default='ctd', help='save results to project/name')
...
if __name__ == "__main__":
    ...
    opt = parse_opt()
    main(opt)
    print('all time use:', time.time() - sss)
