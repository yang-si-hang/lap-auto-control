    ...
    if 1 >0:
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                imc = im0.copy() if True else im0  # for save_crop 之前是if save_crop else +++++++++++修改过+++++++++++++++++=
                ...
                if save_img:
                    cv2.putText(im0, 'index:' + str(all_count), (10, 70), cv2.FONT_HERSHEY_PLAIN, 3,(0, 0, 255), 3)
                    all_count += 1
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'run/train/wyh/weights/best.pt',
                        help='model path(s)')
    ...
    parser.add_argument('--name', default='box', help='save results to project/name')
